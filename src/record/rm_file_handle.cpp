...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    auto record = std::make_unique<RmRecord>(record_size, page_handle.slots + rid.slot_no);
    return record;
}
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    auto pos = Bitmap::first_bit(1, page_handle.bitmap, page_handle.file_hdr->bitmap_size);
    // 3. 将buf复制到空闲slot位置
    memcpy(page_handle.get_slot(pos), buf, page_handle.file_hdr->record_size);
    ...
    return Rid{page_handle.page->get_page_id().page_no, pos};
}
