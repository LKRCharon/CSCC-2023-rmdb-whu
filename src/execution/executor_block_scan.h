...
class BlockScannerNoPush {
   ...
   public:
    BlockScannerNoPush(std::unique_ptr<AbstractExecutor> scanner, Page *pages, int size) : pages_(pages), size_(size) {
        scanner_ = std::move(scanner);
        file_hdr_ = scanner_->GetFileHandle()->get_file_hdr();
        bbm_ = new BlockBufferManager(scanner_->GetFileHandle(), pages, size);
        max_n_ = file_hdr_.num_records_per_page;
    ...
    ~BlockScannerNoPush() { delete bbm_; }

    void bbmNext() { bbm_->next(); }

    // bbm不会扫空页进来，所以begin一定有数据
    void beginTuple() {
        is_buffer_end = false;
        position_ = 0;
